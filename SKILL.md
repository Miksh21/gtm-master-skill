---
name: gtm-master
description: Master GTM / B2B outbound knowledge base: one router over 15 clusters of runnable tasks and deep knowledge. Covers cold email (first emails, follow-ups, subject lines, stress tests, spintax, deliverability, SPF/DKIM/DMARC, warmup), buying signals and intent data (scoring, stacking, alpha signals), list building (Sales Navigator, TAM, ABM), Clay enrichment and workflows (waterfalls, Claygent, Sending Gate, CRM and sequencer push), ICP, personas, lead scoring, objection handling, multi-channel and LinkedIn sequences, campaign ops (hypothesis matrix, A/B tests, reports), RevOps (forecasting, routing, CRM migration, pricing), B2B paid ads (Meta, LinkedIn ABM, Google), AEO and Reddit, content marketing (newsletters, SEO, creators, affiliates) and GTM philosophy (BIPSY). Use for ANY GTM, outbound, sales engineering or RevOps work. Not for deep organic LinkedIn content (linkedin-content), LinkedIn ads fundamentals (linkedin-ads) or exec comms framing (game-pyramid).
---
...
Knowledge index: **human voice** (the final pass on every message: say-it-aloud test, rough edges without errors, no sequence-narrating openers, mirroring, reader's verbs, defensible claims, sender integrity) → `{SKILL_BASE}/clusters/cold-email/knowledge/human-voice.md`; 13 named copywriting frameworks, ATL/BTL messaging, ColdIQ playbook, e-commerce playbook, sequence theory, deliverability deep guide, email infra (3 files), personalization prompt library, campaign playbooks, email template library. Browse `{SKILL_BASE}/clusters/cold-email/knowledge/` for the full map.
...
- **Sequence and system hygiene** — editing live sequences (sibling copies, snapshot/diff, populated conditionals, clone-from-newest, modernize-on-touch), channel mechanics, one-page campaign docs + trigger/angle registries, canonical shared enrichment logic with drift checks, docs-follow-the-system → `{SKILL_BASE}/clusters/campaign-ops/knowledge/sequence-and-system-hygiene.md`
